   ...
   if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <span className="text-xs text-gray-500 mt-3">{t('gerrit.selector.loading')}</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {instances.length === 0 ? (
        <div className="text-center py-8 bg-editor-line/20 rounded-lg border border-editor-line/50">
          ...
          <div className="text-sm text-gray-400">{t('gerrit.selector.no_instances')}</div>
          <div className="text-xs text-gray-500 mt-1">
            {t('gerrit.selector.configure_first')}
          </div>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 text-[10px] text-gray-600">
                  <span>{t('gerrit.selector.user')}: {instance.username}</span>
                  <span>•</span>
      ...
      <div className="flex gap-2 pt-3 border-t border-editor-line mt-1">
        ...
        >
          <Plus size={14} />
          {t('gerrit.selector.configure_new')}
        </button>
