...
const FileTreeItem: React.FC<{
  ...
  expandAll?: boolean;
  selectedFile?: string | null;
}> = ({ node, depth = 0, onSelect, expandAll = false, selectedFile }) => {
  ...
  // Auto-expand if selectedFile is in this folder or its children
  useEffect(() => {
    if (node.type === 'folder' && selectedFile) {
      // Check if selectedFile is a direct child
      const hasSelectedChild =
        hasChildren &&
        node.children!.some((child) =>
          child.type === 'file'
            ? child.path === selectedFile
            : selectedFile.startsWith(child.path + '/'),
        );
  ...
  }, [selectedFile, node, hasChildren, expanded]);
  ...
  return (
    <div>
      <div
        className={`flex items-center gap-1.5 py-1 hover:bg-editor-line/50 cursor-pointer select-none transition-colors border-l-4 ${
          isSelected
            ? 'bg-editor-accent/30 border-editor-accent shadow-[0_0_12px_rgba(59,130,246,0.4)]'
            : 'border-transparent'
        }`}
      ...
      >
        ...
        {node.type === 'folder' ? (
          <Folder
            size={14}
            className={`${isSelected ? 'text-editor-accent' : 'text-editor-accent'} shrink-0`}
          />
        ) : (
          <FileCode
            size={14}
            className={`${isSelected ? 'text-editor-accent' : getStatusColor(node.status)} shrink-0`}
          />
        )}

        <span
          className={`text-xs truncate ${node.type === 'folder' ? 'text-editor-fg font-medium' : 'text-gray-400'} ${isSelected ? 'text-white font-bold' : ''} ${!node.exists && node.type === 'file' ? 'opacity-60' : ''}`}
        >
          {node.name}
        </span>
        ...
        {/* Selected indicator */}
        {isSelected && (
          <span className="ml-auto mr-2 flex items-center gap-2">
            <span className="w-2 h-2 bg-editor-accent rounded-full animate-pulse shadow-[0_0_8px_rgba(59,130,246,1)]"></span>
            <span className="text-[10px] text-editor-accent font-bold">●</span>
          </span>
        )}
        ...
        {node.stats && (
          <span className="ml-auto mr-2 text-[10px] text-gray-600 flex gap-1">
            {node.stats.added > 0 && (
              <span className="text-editor-success">+{node.stats.added}</span>
            )}
            {node.stats.removed > 0 && (
              <span className="text-editor-error">-{node.stats.removed}</span>
            )}
          </span>
        )}
      </div>
...
const TaskTree: React.FC<TaskTreeProps> = ({
  ...
  return (
    <div
      id="tour-task-tree"
      className="h-full bg-editor-sidebar border-r border-editor-line flex flex-col"
    >
      {/* Tabs */}
                      ...
                      {pendingTasks.map((task) => (
                        ...
                        >
                          {task.id === activeTaskId ? (
                            ...
                            <Circle
                              size={10}
                              className="text-editor-error fill-editor-error shrink-0"
                            />
                          ) : (
                    ...
                    <div className="flex flex-col">
                      {watchedTasks.map((task) => (
                        ...
                        >
                          ...
                          <span className="truncate">{task.title}</span>
                          {task.unreadCount && (
                            <span className="ml-auto text-[10px] bg-editor-error text-white px-1 rounded-full">
                              {task.unreadCount}
                            </span>
                          )}
                        </div>
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto">
                  {sortedLocalTasks.length === 0 ? (
                    <div className="px-4 py-2 text-xs text-gray-500 italic">
                      No local tasks. Create one above.
                    </div>
                  ) : (
                        ...
                        <div key={task.id} className="flex flex-col">
                          ...
                          >
                            ...
                            {task.status === 'completed' ? (
                              ...
                              <Circle
                                size={12}
                                className="text-editor-accent fill-editor-accent/20 shrink-0"
                              />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0 flex items-center gap-2">
                              <div className="flex flex-col gap-0.5">
                                <span
                                  className={`truncate font-medium ${task.id === activeTaskId ? 'text-white' : 'text-gray-400 group-hover:text-gray-300'}`}
                                >
                                  {task.title}
                                </span>
                                {task.type && (
                                  <span
                                    className={`text-[9px] px-1.5 py-0 rounded border w-fit uppercase font-bold ${getTypeBadgeColor(task.type)}`}
                                  >
                                    {task.type}
                                  </span>
                                )}
                              ...
                              <div className="ml-auto flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                ...
                                <div
                                  className="relative"
                                  ref={exportMenuOpen === task.id ? exportMenuRef : null}
                                >
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setExportMenuOpen(
                                        exportMenuOpen === task.id ? null : task.id,
                                      );
                                    }}
                          ...
                          {isExpanded && task.files && (
                            <div className="flex flex-col bg-black/10 pb-1">
                              {task.files.map((file) => {
                                ...
                                const getReviewStatusIcon = (status: FileReviewStatus) => {
                                  switch (status) {
                                    case 'approved':
                                      return (
                                        <CheckCircle size={12} className="text-editor-success" />
                                      );
                                    case 'concern':
                                      return (
                                        <AlertTriangle size={12} className="text-editor-warning" />
                                      );
                                    case 'must_change':
                                      return <XCircle size={12} className="text-editor-error" />;
                                    case 'question':
                                      return (
                                        <MessageSquare size={12} className="text-editor-accent" />
                                      );
                                    default:
                                ...
                                const getReviewStatusColor = (status: FileReviewStatus) => {
                                  switch (status) {
                                    case 'approved':
                                      return 'bg-editor-success/20 text-editor-success';
                                    case 'concern':
                                      return 'bg-editor-warning/20 text-editor-warning';
                                    case 'must_change':
                                      return 'bg-editor-error/20 text-editor-error';
                                    case 'question':
                                      return 'bg-editor-accent/20 text-editor-accent';
                                    default:
                                ...
                                return (
                                  ...
                                  >
                                    <File
                                      size={12}
                                      className="text-gray-500 group-hover/file:text-editor-accent"
                                    />
                                    <span
                                      className={`text-xs truncate text-gray-500 group-hover/file:text-gray-300 ${file.status === 'modified' ? 'text-editor-warning' : file.status === 'added' ? 'text-editor-success' : ''}`}
                                    >
                                      {file.name}
                                    </span>
                                    <span className="ml-auto flex items-center gap-2">
                                      {/* File status badge */}
                                      <span
                                        className={`px-1.5 py-0 rounded text-[9px] font-medium border ${getReviewStatusColor(reviewStatus)}`}
                                      >
                              ...
                              {task.files.length === 0 && (
                                <div className="pl-9 text-[10px] text-gray-600 italic py-1">
                                  No files attached
                                </div>
                              )}
                            </div>
