...
const LocalRightPanel: React.FC<LocalRightPanelProps> = () => {
    ...
    <div className="h-full bg-editor-sidebar border-l border-editor-line flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                ...
                <div className="space-y-3">
                    {heatmap.map(item => (
                        <div key={item.id} className="flex items-center gap-3 p-2 bg-editor-line/20 rounded border border-transparent hover:border-editor-line cursor-pointer transition-all group">
                            <div className={`w-1 h-8 rounded-full ${item.impact === 'High' ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]' : item.impact === 'Medium' ? 'bg-yellow-500' : 'bg-blue-500'}`} />
        ...
        {activeTab === LocalTab.KB && (
            <section className="animate-fade-in">
                <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <BookOpen size={14} className="text-editor-accent" />
                    {t('rightpanel.knowledge_base')}
                </h3>
                <div className="text-center text-gray-500 text-sm py-8">
                    Knowledge base content will be added here
                </div>
            </section>
        )}
      </div>
