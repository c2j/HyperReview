...
const DiffView: React.FC<DiffViewProps> = ({
  ...
  const fileExtension = useMemo(() => {
    if (filePath === 'current-file') {
      // Use activeFilePath for default
      const defaultPath = _activeFilePath || 'src/main/RetryService.java';
      const parts = defaultPath.split('.');
      return parts.length > 1 ? parts[parts.length - 1].toLowerCase() : 'java';
    }
    const parts = filePath.split('.');
    return parts.length > 1 ? parts[parts.length - 1].toLowerCase() : '';
  }, [filePath, _activeFilePath]);

  // Get language name based on extension
  const languageName = useMemo(() => {
    ...
    return langMap[fileExtension] || fileExtension.toUpperCase();
  }, [fileExtension]);

  // Check if file is Java (for Lombok folding)
  const isJavaFile = fileExtension === 'java';
  ...
  useEffect(() => {
    console.log(
      '[DiffView] useEffect triggered with filePath:',
      filePath,
      'isRepoLoaded:',
      isRepoLoaded,
    );
  ...
  const handleMenuAction = (action: string) => {
    if (onAction) {
      onAction(
        `Context Action: ${action} on line ${contextMenu.lineIndex !== null ? diffLines[contextMenu.lineIndex]?.new_line_number || 'context' : 'unknown'}`,
      );
    }
    closeContextMenu();
  };
    ...
    <div id="tour-diff-view" className="h-full bg-editor-bg flex flex-col min-w-0 relative">
      ...
      <div className="h-[36px] bg-editor-bg border-b border-editor-line flex items-center px-4 justify-between shrink-0 relative z-20">
        <div className="flex items-center gap-2 text-xs truncate mr-4">
          ...
          <span className="text-editor-fg font-medium truncate">
            {filePath === 'current-file' ? _activeFilePath : filePath}
          </span>
          {showFileContent ? (
            <span className="text-editor-info text-[10px] ml-2 border border-editor-info/30 px-1 py-0.5 rounded">
              FILE
            </span>
          ) : (
            <>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative group mr-1">
            <button
              className="flex items-center gap-1.5 px-2 py-0.5 rounded hover:bg-editor-line text-xs text-gray-400 hover:text-white transition-colors"
              title={t('diffview.view.diff')}
            >
              <Eye size={14} />
              <span className="hidden lg:inline">
                {viewMode === 'diff'
                  ? t('diffview.view.diff')
                  : viewMode === 'old'
                    ? t('diffview.view.old')
                    : t('diffview.view.new')}
              </span>
            ...
            <div className="absolute right-0 top-full w-32 pt-2 hidden group-hover:block z-50">
              <div className="bg-editor-sidebar border border-editor-line rounded shadow-xl py-1">
                <div
                  ...
                  className={`px-3 py-1.5 text-xs cursor-pointer hover:bg-editor-line ${viewMode === 'diff' ? 'text-white' : 'text-gray-400'}`}
                >
                  {t('diffview.view.diff')}
                </div>
                <div
                  onClick={() => setViewMode('old')}
                  className={`px-3 py-1.5 text-xs cursor-pointer hover:bg-editor-line ${viewMode === 'old' ? 'text-white' : 'text-gray-400'}`}
                >
                  {t('diffview.view.old')}
                </div>
                <div
                  onClick={() => setViewMode('new')}
                  className={`px-3 py-1.5 text-xs cursor-pointer hover:bg-editor-line ${viewMode === 'new' ? 'text-white' : 'text-gray-400'}`}
                >
                  {t('diffview.view.new')}
                </div>
              </div>
          ...
          {/* Fold Lombok - Only for Java files */}
          {isJavaFile && (
            <button
          ...
          <span className="text-[10px] text-editor-accent uppercase font-bold ml-1 hidden xl:inline">
            {languageName}
          </span>
        </div>
      </div>
        ...
        <div className="absolute top-[40px] right-6 z-50 bg-editor-sidebar border border-editor-line shadow-xl rounded flex items-center p-1 gap-1 animate-fade-in-down">
          ...
          <div className="text-[10px] text-gray-500 min-w-[50px] text-center font-mono">
            {matches.length > 0
              ...
              : t('diffview.search.no_results')}
          </div>
          <button
            onClick={handlePrevMatch}
            disabled={matches.length === 0}
            className="p-1 hover:bg-editor-line rounded text-gray-400 hover:text-white disabled:opacity-30"
          >
            <ArrowUp size={14} />
          </button>
          <button
          ...
          <button
            onClick={toggleSearch}
            className="p-1 hover:bg-editor-line rounded text-gray-400 hover:text-white ml-1"
          >
            <X size={14} />
          </button>
        </div>
      ...
      {contextMenu.visible && (
        ...
        >
          <div
            ...
            className="flex items-center gap-3 px-3 py-1.5 text-xs text-editor-fg hover:bg-editor-line hover:text-white cursor-pointer transition-colors"
          >
            <XCircle size={14} className="text-editor-error" />
            <span>{t('contextmenu.must_change')}</span>
          </div>
          <div
            onClick={() => handleMenuAction('Concern')}
            className="flex items-center gap-3 px-3 py-1.5 text-xs text-editor-fg hover:bg-editor-line hover:text-white cursor-pointer transition-colors"
          >
          ...
          <div
            onClick={() => handleMenuAction('ApproveHunk')}
            className="flex items-center gap-3 px-3 py-1.5 text-xs text-editor-fg hover:bg-editor-line hover:text-white cursor-pointer transition-colors"
          >
          ...
          <div
            onClick={() => handleMenuAction('Generate Patch')}
            className="flex items-center gap-3 px-3 py-1.5 text-xs text-editor-fg hover:bg-editor-line hover:text-white cursor-pointer transition-colors"
          >
      ...
      <div className="flex items-center bg-editor-sidebar border-b border-editor-line text-[10px] text-gray-500 font-bold uppercase tracking-wider select-none shrink-0 z-10 sticky top-0">
        {(viewMode === 'diff' || viewMode === 'old') && (
          <div
            className={`w-[60px] text-right pr-3 py-1 border-r border-editor-line/30 bg-editor-bg/50 text-red-400/70 truncate ${viewMode === 'old' ? 'w-[60px]' : ''}`}
        ...
        {viewMode === 'diff' && (
          <div
            className="w-[12px] flex justify-center py-1 bg-editor-bg/50"
            title="Changes Heatmap"
          >
        ...
        {(viewMode === 'diff' || viewMode === 'new') && (
          <div
            className="w-[60px] text-right pr-3 py-1 border-r border-editor-line/30 bg-editor-bg/50 text-green-400/70 truncate"
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="bg-editor-bg/50 border border-editor-line/30 rounded p-3 mb-4">
                      ...
                      <code className="text-xs text-editor-accent font-mono break-all">
                        {filePath}
                      </code>
                    </div>
                    {diffContext && (
