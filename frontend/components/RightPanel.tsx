...
const RightPanel: React.FC<RightPanelProps> = ({ onAction, activeFileExtension = '', repoRefreshKey, onSelectFile }) => {
                                ...
                                <div className="flex-1 min-w-0">
                                     <div className="flex justify-between items-center mb-0.5">
                                         <div className="flex items-center gap-1.5 min-w-0">
                                             <span className={`text-xs font-medium text-gray-300 group-hover:text-white truncate ${!item.exists ? 'opacity-60' : ''}`}>{item.name}</span>
                                     ...
                                     <div className={`text-[10px] text-gray-600 truncate font-mono ${!item.exists ? 'opacity-60' : ''}`}>{item.path}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="relative flex-1">
                                ...
                                >
                                    ...
                                    {/* 动态生成分类选项 */}
                                    {Array.from(new Set(guideItems.map(item => item.category))).sort().map(cat => (
