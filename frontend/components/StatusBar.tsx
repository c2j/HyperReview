  ...
  return (
    <div className="h-[28px] bg-editor-accent text-white flex items-center px-4 justify-between text-[11px] select-none shrink-0 z-50 overflow-hidden whitespace-nowrap">
      ...
      <div className="flex items-center gap-4 min-w-0 flex-1 mr-4">
        ...
        <span
          className="opacity-80 truncate min-w-[50px]"
          title={
            currentRepo ? `${currentRepo.path}/${currentRepo.current_branch}` : 'No repository'
          }
        >
          {currentRepo
            ? `[${currentRepo.path}/${currentRepo.current_branch}]`
        ...
        {stats ? (
          <>
            <span className="shrink-0">
              {stats.total_files} {t('statusbar.files')}
            </span>
            <span className="shrink-0">→</span>
            <span className="shrink-0 truncate">
              {t('statusbar.marked')} {stats.reviewed_files} (✗{stats.severe_issues || 0} ⚠
              {(stats.total_comments || 0) - (stats.severe_issues || 0)} ❓{stats.pending_files})
            </span>
          </>
        ) : (
      ...
      <div className="flex items-center gap-4 shrink-0">
        <span className="hidden md:block truncate max-w-[200px] opacity-80">
          {t('statusbar.ready')}
        </span>
