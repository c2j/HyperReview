use crate::models::task::{LocalTask, CreateTaskRequest, TaskSummary, TaskStatus};
...
pub async fn submit_task_to_gerrit(
    ...
    use crate::models::task::Comment;
    
    let store = TaskStore::new().map_err(|e| e.to_string())?;
    let task = store.load_task(uuid::Uuid::parse_str(&task_id).unwrap())
        .map_err(|e| e.to_string())?;

    let comments: Vec<crate::models::task::Comment> = task.items
        .into_iter()
        .filter_map(|item| item.comments.into_iter().next())
        .collect();

    let gerrit_client = GerritClient::new("https://gerrit-review.example.com");
    let result = gerrit_client.submit_review(&change_id, &revision_id, comments, score)
        .map_err(|e| e.to_string())?;
...
pub async fn submit_task_to_codearts(
    ...
    let result = codearts_client.submit_review(&project_id, mr_id, comments, None)
        .map_err(|e| e.to_string())?;
