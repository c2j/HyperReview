#[cfg(test)]
mod tests {
    use crate::commands::task_commands::{export_task, export_all_tasks};
    use crate::models::task::{LocalTask, TaskStatus, TaskItem, Comment};
    ...
    async fn create_test_task(name: &str, status: TaskStatus, completed_items: u32, total_items: u32) -> LocalTask {
    ...
    #[tokio::test]
    async fn test_export_single_task() {
        setup_test_environment();
        let task = create_test_task("Test Task", TaskStatus::InProgress, 1, 3).await;
        let task_id = task.id;

        let result = export_task(task_id).await;
    ...
    #[tokio::test]
    async fn test_export_completed_task() {
        setup_test_environment();
        let task = create_test_task("Completed Task", TaskStatus::Completed, 5, 5).await;
        let task_id = task.id;
    ...
    #[tokio::test]
    async fn test_export_archived_task() {
        setup_test_environment();
        let task = create_test_task("Archived Task", TaskStatus::Archived, 2, 4).await;
        let task_id = task.id;
    ...
    #[tokio::test]
    async fn test_export_task_with_unicode() {
        setup_test_environment();
        ...
        assert_eq!(parsed["items"][0]["description"], "审查代码");
    ...
    #[tokio::test]
    async fn test_export_task_with_special_characters() {
        setup_test_environment();
        ...
        assert_eq!(parsed["items"][0]["description"], "Handle special cases: @#$%^&*()");
        
        cleanup_test_environment();
    }

    #[tokio::test]
    async fn test_export_nonexistent_task() {
        setup_test_environment();

        let nonexistent_task_id = Uuid::new_v4();
        
        let result = export_task(nonexistent_task_id).await;
    ...
    #[tokio::test]
    async fn test_export_all_tasks_empty() {
        setup_test_environment();

        let result = export_all_tasks().await;
    ...
    #[tokio::test]
    async fn test_export_all_tasks_multiple() {
        setup_test_environment();
        
        let task1 = create_test_task("Task 1", TaskStatus::InProgress, 2, 4).await;
        let task2 = create_test_task("Task 2", TaskStatus::Completed, 3, 3).await;
        let task3 = create_test_task("Task 3", TaskStatus::Archived, 1, 5).await;
        ...
        let task1_data = parsed.as_array().unwrap().iter()
            ...
            .unwrap();
        assert_eq!(task1_data["status"], "InProgress");
    ...
    #[tokio::test]
    async fn test_export_task_json_format() {
        setup_test_environment();
        let task = create_test_task("Format Test Task", TaskStatus::InProgress, 1, 2).await;
        let task_id = task.id;
    ...
    #[tokio::test]
    async fn test_export_task_with_empty_items() {
        setup_test_environment();
    ...
    #[tokio::test]
    async fn test_export_task_preserves_timestamps() {
        setup_test_environment();
