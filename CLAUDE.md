...
- TypeScript 5+, React 18+, Rust 1.75+, Tauri v2 + Vite (build), git2-rs (git operations), rusqlite (metadata), tree-sitter (code analysis), Zustand (state) (003-local-task-management)
- Local JSON file system (~/.hyperreview/local_tasks/) for tasks, SQLite via rusqlite for metadata (003-local-task-management)
...
## Recent Changes
- 003-local-task-management: Added TypeScript 5+, React 18+, Rust 1.75+, Tauri v2 + Vite (build), git2-rs (git operations), rusqlite (metadata), tree-sitter (code analysis), Zustand (state)
